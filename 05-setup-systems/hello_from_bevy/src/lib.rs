...
use bevy::{
    MinimalPlugins,
    app::{
        App, AppExit, AppExitWithHotReload, Last, Main, PluginGroup, ScheduleRunnerPlugin, Startup,
        Update,
    },
    ecs::{
...
pub extern "C" fn create_app() -> *mut App {
    ...
    app.add_plugins(MinimalPlugins.set(ScheduleRunnerPlugin::run_loop(Duration::from_secs(1))))
        .add_systems(Startup, startup_hi)
...
pub unsafe extern "C" fn hot_reload_reconciliation(
    ...
    if let Some(new_main_schedule) = updated_schedules.get_mut(Main) {
        let mut old_schedules = old_app.world_mut().resource_mut::<Schedules>();
